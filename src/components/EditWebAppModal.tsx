...
import { AdminWebApp, ServerCategory } from '../types.ts';
import { CategoryBadge } from './CategoryBadge.tsx';

interface ServerEditDraft {
  id?: string;
  tempId: string;
  url: string;
...
export const EditWebAppModal: React.FC<EditWebAppModalProps> = ({
  webApp,
  isOpen,
  onClose,
  onSave,
  ...
  const [servers, setServers] = useState<ServerEditDraft[]>([]);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Sync state whenever webApp changes
  useEffect(() => {
    if (webApp) {
      setName(webApp.name);
      if (webApp.icon.startsWith('data:image/')) {
        setIconMode('upload');
        setUploadedImage(webApp.icon);
        setIconUrl('');
      } else {
        setIconMode('url');
        setIconUrl(webApp.icon);
        setUploadedImage(null);
      }

      setServers(
        webApp.servers.map((s, idx) => ({
          id: s.id,
  ...
  const handleRemoveServer = (index: number) => {
    ...
    // Removing automatically renumbers Server 1, Server 2...
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Validate Name
    if (!name.trim()) {
      setError('Web App name is required.');
      return;
    }
    ...
    for (let i = 0; i < servers.length; i++) {
      ...
      try {
        ...
        setError(`Server ${i + 1}: Please enter a valid URL (http:// or https://)`);
        return;
      }
    }
    ...
    try {
      ...
      setError(err.message || 'Failed to update Web App');
    } finally {
      setIsSubmitting(false);
    }
  };

  if (!isOpen || !webApp) return null;

  return (
    <AnimatePresence>
      <div
        id="edit-webapp-modal-backdrop"
      ...
      >
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50/70 dark:bg-neutral-900/60 shrink-0">
            <div>
              <h2 id="edit-modal-title" className="text-xl font-bold text-neutral-900 dark:text-neutral-100 tracking-tight">
                Edit Web App
              </h2>
              <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                Update app name, icon, and manage server routes
              </p>
            </div>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
            {error && (
              <div id="edit-form-error" className="p-3.5 rounded-xl bg-rose-50 dark:bg-rose-950/40 border border-rose-200 dark:border-rose-900/50 text-rose-700 dark:text-rose-300 text-xs font-medium">
                {error}
              </div>
            )}
            ...
            {/* Web App Name */}
            <div>
              <label htmlFor="edit-app-name-input" className="block text-xs font-semibold text-neutral-700 dark:text-neutral-300 uppercase tracking-wider mb-2">
                Web App Name *
              </label>
              <input
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start">
                ...
                <div className="flex-1 w-full space-y-2">
                  {iconMode === 'url' ? (
                    ...
                    <div>
                      <label
                        htmlFor="edit-app-icon-file-input"
                      ...
                      >
                        ...
                        <span className="text-xs font-medium text-neutral-700 dark:text-neutral-300">
                          {uploadedImage ? 'Replace uploaded image' : 'Click to browse or drag and drop'}
                        </span>
              ...
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <span className="text-[11px] text-neutral-400">
                    Automatically derives Server 1, Server 2... numbering
                  </span>
                </div>

                <button
                  id="edit-add-server-btn"
                  type="button"
